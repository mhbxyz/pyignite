...
def build_fastapi_template(context: FastAPITemplateContext) -> dict[Path, str]:
    ...
    files: dict[Path, str] = {
        ...
        src_package / "api" / "health.py": _health_module(),
        src_package / "api" / "router.py": _router_module(),
        Path("tests") / "__init__.py": _package_init(),
        Path("tests") / "test_health.py": _health_test_module(context),
    }
...
Generated by PyQuick using the FastAPI profile (no DB).
...
name = "{project_slug}"
version = "0.1.0"
description = "FastAPI service generated by PyQuick"
readme = "README.md"
...
app = FastAPI(title="PyQuick API")
app.include_router(health_router)
app.include_router(router)
"""


def _health_module() -> str:
...
def root() -> dict[str, str]:
    return {"status": "ok", "message": "PyQuick scaffold is running"}
"""


def _health_test_module(context: FastAPITemplateContext) -> str:
